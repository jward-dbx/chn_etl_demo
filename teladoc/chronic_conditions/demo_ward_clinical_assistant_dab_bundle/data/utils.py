...
def save_to_parquet(df: pd.DataFrame, table_name: str, num_files: int = 5) -> None:
  ...
  bronze_schema = os.getenv('BRONZE_SCHEMA', os.getenv('SCHEMA', 'bronze'))  # Support both BRONZE_SCHEMA and SCHEMA for backward compat
  volume = os.getenv('VOLUME')

  if catalog and bronze_schema and volume:
    ...
    outdir = f'/Volumes/{catalog}/{bronze_schema}/{volume}/{table_name}'
  else:
  ...
  n = len(df)
  location = f'{catalog}.{bronze_schema}.{volume}' if (catalog and bronze_schema and volume) else 'local filesystem'
  ...
  if catalog and bronze_schema and volume:
    try:
      ...
      print(f"Creating Delta table: {catalog}.{bronze_schema}.{table_name}")
      spark.sql(create_table_sql)
      print(f"✓ Delta table {catalog}.{bronze_schema}.{table_name} created successfully")
...
def save_to_table(df: pd.DataFrame, table_name: str, num_files: int = 5) -> None:
  """Write DataFrame directly to a Delta table.
    - Converts pandas DataFrame to Spark DataFrame
    - Writes directly to Delta table in the specified catalog and schema
    - Handles datetime columns appropriately for Delta tables
  ...
  catalog = os.getenv('CATALOG')
  bronze_schema = os.getenv('BRONZE_SCHEMA', os.getenv('SCHEMA', 'bronze'))
  
  if not catalog or not bronze_schema:
    raise ValueError("CATALOG and BRONZE_SCHEMA (or SCHEMA) environment variables must be set")
  ...
  try:
    ...
    if bronze_schema not in schemas:
      print(f"Creating schema: {catalog}.{bronze_schema}")
      spark.sql(f"CREATE SCHEMA IF NOT EXISTS {catalog}.{bronze_schema}")
    else:
      print(f"Schema {catalog}.{bronze_schema} already exists ✅")
  except Exception as e:
    print(f"Warning: Could not ensure schema exists: {e}")
    print("Proceeding with write attempt...")

  # Normalize datetime columns to milliseconds precision (tz-naive)
  # This ensures compatibility with Delta TIMESTAMP_NTZ
  df_copy = df.copy()
  for c in _datetime_cols(df_copy):
    s = pd.to_datetime(df_copy[c], errors='coerce').dt.floor('ms')
    df_copy[c] = s.dt.tz_localize(None).astype('datetime64[ms]')
  ...
  print(f"Writing {n:,} rows to Delta table: {full_table_name}")
  
  # Write with specified number of partitions
  spark_df.coalesce(num_files).write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(full_table_name)
  
  print(f"✓ Saved {n:,} rows to Delta table {full_table_name} ({num_files} partitions)")
