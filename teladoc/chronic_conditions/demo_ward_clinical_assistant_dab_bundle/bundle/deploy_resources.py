...
# MAGIC %md
# MAGIC # Deploy Agent Bricks Resources
# MAGIC 
# MAGIC This notebook deploys Genie spaces, Knowledge Assistants, and Multi-Agent Supervisors from configuration.
...
# Databricks notebook source
# MAGIC %md
# MAGIC ## Import Agent Bricks Service

# COMMAND ----------
# Import agent_bricks_service from the same directory
try:
    from agent_bricks_service import AgentBricksManager, get_tile_example_queue
except ImportError:
    # If import fails, load from workspace path
    import importlib.util
    import sys
    
    # Get the current notebook path and construct agent_bricks_service path
    notebook_path = dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get()
    base_path = '/'.join(notebook_path.split('/')[:-1])
    service_path = f"/Workspace{base_path}/agent_bricks_service.py"
    
    if os.path.exists(service_path):
        spec = importlib.util.spec_from_file_location("agent_bricks_service", service_path)
        agent_bricks_service = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(agent_bricks_service)
        AgentBricksManager = agent_bricks_service.AgentBricksManager
        get_tile_example_queue = agent_bricks_service.get_tile_example_queue
    else:
        # Fallback: use workspace sync location
        workspace_path = os.getenv('WORKSPACE_PATH', '/Users/justin.ward@databricks.com/demo_ward_clinical_assistant')
        service_path = f"{workspace_path}/bundle/agent_bricks_service.py"
        if os.path.exists(service_path):
            spec = importlib.util.spec_from_file_location("agent_bricks_service", service_path)
            agent_bricks_service = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(agent_bricks_service)
            ...
            raise ImportError("Could not find agent_bricks_service.py")
...
def main():
    ...
    # Read configuration from bundle directory
    # Try __file__ first, fall back to cwd
    try:
        # If running as script, use __file__ location
        bundle_dir = Path(__file__).parent
        bricks_conf_path = bundle_dir / 'bricks_conf.json'
    except NameError:
        # If __file__ not available (Databricks jobs), use workspace path
        workspace_path = os.getenv('WORKSPACE_PATH', '/Users/justin.ward@databricks.com/demo_ward_clinical_assistant')
        bricks_conf_path = Path(workspace_path) / 'bundle' / 'bricks_conf.json'
